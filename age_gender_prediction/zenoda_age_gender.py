...
Using audonnx (Zenoda) model
====================================================

Pipeline:
    1. pyannote/speaker-diarization-3.1 splits the audio into speaker turns.
    2. For every speaker, several *independent* speech segments are fed 
       into audonnx age/gender model.
    3. Per-segment predictions are combined into a single robust estimate
       per speaker using duration-weighted statistics.
    4. Speakers whose voice is judged to belong to a minor are reported as
...
# Zenoda model URL
ZENODA_MODEL_URL = 'https://zenodo.org/record/7761387/files/w2v2-L-robust-6-age-gender.25c844af-1.1.1.zip'
ZENODA_MODEL_PATH = './zenoda_model'
...
def collect_speaker_segments(annotation) -> Dict[str, List[dict]]:
    """All diarized turns per speaker, unfiltered by length."""
...
# ============================================================
# 7. INFERENCE WITH ZENODA MODEL
# ============================================================

def load_zenoda_model():
    """Download and load the Zenoda (audonnx) model"""
    log.info("Loading Zenoda model from: %s", ZENODA_MODEL_URL)
    
    if not os.path.exists(ZENODA_MODEL_PATH):
        log.info("Downloading model...")
        os.makedirs(ZENODA_MODEL_PATH, exist_ok=True)
        archive_path = audeer.download_url(ZENODA_MODEL_URL, './')
        audeer.extract_archive(archive_path, ZENODA_MODEL_PATH)
        log.info("Model downloaded!")
    
    model = audonnx.load(ZENODA_MODEL_PATH)
    log.info("Zenoda model loaded!")
    return model


def predict_segment_zenoda(clip: np.ndarray, model) -> Optional[SegmentPrediction]:
    """Predict age and gender for a single audio segment using Zenoda model"""
    try:
        # Ensure clip is float32
        clip = clip.astype(np.float32)
        
        # Run inference
        output = model(clip, SAMPLE_RATE)
        
        # Parse age
        age_score = float(output['logits_age'].flatten()[0])
        estimated_age = np.clip(age_score * 100.0, 0.0, 100.0)
        age_code, _ = age_to_group(estimated_age)
        
        # Parse gender
        gender_scores = output['logits_gender'].flatten()
        gender_probs = np.exp(gender_scores) / np.sum(np.exp(gender_scores))  # softmax
        ...
        log.warning("Segment prediction failed: %s", e)
        return None
...
def process_speaker(speaker: str, segments: List[dict], audio: np.ndarray, sr: int,
    ...
    for seg in analysis_segments:
        ...
        if clip is None:
            continue
        try:
            pred = predict_segment_zenoda(clip, zenoda_model)
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Speaker diarization + age/gender estimation using Zenoda")
    ...
    zenoda_model = load_zenoda_model()
    ...
    print()
    print("=" * 70)
    print("SPEAKER AGE GROUP / GENDER ESTIMATION (Zenoda Model)")
    print("=" * 70)
    for speaker, segments in speaker_segments.items():
        result = process_speaker(speaker, segments, audio, SAMPLE_RATE, zenoda_model)
