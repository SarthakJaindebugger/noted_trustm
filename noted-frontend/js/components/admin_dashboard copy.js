  ...
  setup() {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        topicsDiscussed.value = data.topics_discussed || [];
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50">
      ...
      <div class="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div class="glass-card p-6 rounded-2xl">
            <div class="text-gray-500">Age Groups</div>
            <div class="text-sm mt-2" v-if="ageGroups.length">
              <div v-for="g in ageGroups" :key="g.range">{{ g.range }}: {{ g.pct }}%</div>
            </div>
            <div class="text-gray-400 italic" v-else>—</div>
          </div>
          ...
          <div class="glass-card p-6 rounded-2xl">
            <div class="text-gray-500">Topics Discussed</div>
            <div class="text-sm mt-2" v-if="topicsDiscussed.length">{{ topicsDiscussed.join(', ') }}</div>
            <div class="text-gray-400 italic" v-else>—</div>
          </div>
