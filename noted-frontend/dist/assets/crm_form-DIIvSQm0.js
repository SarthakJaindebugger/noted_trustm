import{a as n,o as w,c as k,r as i}from"./main-BkNScvkK.js";class D{async generateForm(s){return n.post(`/sessions/${s}/crm-form`)}async getForm(s){return n.get(`/sessions/${s}/crm-form`)}async saveForm(s,o){return n.put(`/sessions/${s}/crm-form`,o)}}const c=new D,_={name:"CRMForm",props:{sessionId:{type:String,default:null}},setup(a){const s=i(!0),o=i(!1),e=i(null),r=i(null),l=i(""),u=["in-person","phone","video","email"],m=["resolved","follow-up needed","referred","pending","cancelled"],b=k(()=>{var t;return((t=e.value)==null?void 0:t.status)==="submitted"}),g=async()=>{if(a.sessionId){s.value=!0,r.value=null;try{e.value=await c.getForm(a.sessionId)}catch(t){if(t.status===404)try{e.value=await c.generateForm(a.sessionId)}catch{r.value="Failed to generate CRM form. Make sure the session has a summary."}else r.value="Failed to load CRM form."}finally{s.value=!1}}},f=async(t=!1)=>{if(e.value){o.value=!0,r.value=null,l.value="";try{const d={...e.value};t&&(d.status="submitted"),e.value=await c.saveForm(a.sessionId,d),l.value=t?"Form submitted successfully!":"Draft saved.",setTimeout(()=>{l.value=""},3e3)}catch{r.value="Failed to save form."}finally{o.value=!1}}},p=()=>{e.value.action_items||(e.value.action_items=[]),e.value.action_items.push("")},v=t=>{e.value.action_items.splice(t,1)},x=()=>{e.value.referrals||(e.value.referrals=[]),e.value.referrals.push("")},y=t=>{e.value.referrals.splice(t,1)},h=()=>{window.history.back()};return w(()=>{g()}),{loading:s,saving:o,formData:e,error:r,successMessage:l,encounterTypes:u,outcomeOptions:m,isSubmitted:b,saveForm:f,addActionItem:p,removeActionItem:v,addReferral:x,removeReferral:y,goBack:h}},template:`
