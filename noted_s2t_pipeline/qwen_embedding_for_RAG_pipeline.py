# HF KEY = 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
!pip install faster-whisper huggingface_hub pandas sentence_transformers numpy faiss-cpu pyarrow
...
EMBEDDING_PARQUET = "rag_embeddings.parquet"
...
def build_semantic_index(segments: list) -> tuple:
    """
    Build a FAISS semantic index from transcript segments
    using the globally loaded Qwen embedder.
    """
    ...
    embeddings = qwen_embedder.encode(
        texts,
        convert_to_numpy=True,
        normalize_embeddings=True,
        batch_size=32,
    ...
    return index, texts, qwen_embedder
...
def build_rag_chunks(final_output):

    chunks = []

    metadata = final_output.get("metadata", {})
    extraction = final_output.get("extraction", {})
    ...
    return chunks
def create_embedding_text(chunk, extraction):

    return f"""
    Customer Profile

    Gender:
    {extraction.get("Gender", "Not specified")}

    Age Group:
    {extraction.get("Age_Group", "Not specified")}

    Reason for Immigration:
    {extraction.get("Reason_for_Immigration", "Not specified")}

    Labor Market Position:
    {extraction.get("Labor_Market_Position", "Not specified")}

    Customer Domicile:
    {extraction.get("Customers_Domicile", "Not specified")}

    Duration of Residence in Finland:
    {extraction.get("Duration_of_Residence_in_Finland", "Not specified")}

    Country of Birth:
    {extraction.get("Country_of_Birth", "Not specified")}

    Mother Tongue:
    {extraction.get("Mother_Tongue", "Not specified")}

    Finnish Language Skills:
    {extraction.get("Finnish_Language_Skills", "Not specified")}

    Education Level:
    {extraction.get("Education_Level", "Not specified")}

    Topics Discussed:
    {extraction.get("Topics_Discussed", "Not specified")}

    Number of People in Household:
    {extraction.get("Number_of_People_in_Household", "Not specified")}

    How Customer Heard About Service:
    {extraction.get("How_Customer_Heard_About_Service", "Not specified")}

    Services Sought:
    {extraction.get("Services_Sought", "Not specified")}

    Guidance Provided:
    {extraction.get("Guidance_Provided", "Not specified")}

    Referrals Made:
    {extraction.get("Referrals_Made", "Not specified")}
...
def generate_rag_embeddings(final_output, source_file):
    chunks = build_rag_chunks(final_output)
    extraction = final_output.get("extraction", {})
    records = []
    texts = []

    for chunk in chunks:
        texts.append(create_embedding_text(chunk, extraction))
    ...
    for chunk, emb in zip(chunks, embeddings):
        records.append({
            ...
            "metadata": json.dumps(final_output["metadata"]),
...
def append_embeddings_to_parquet(records):
    df_new = pd.DataFrame(records)

    if os.path.exists(EMBEDDING_PARQUET):
        df_old = pd.read_parquet(EMBEDDING_PARQUET)
        df = pd.concat([df_old, df_new], ignore_index=True)
    else:
        df = df_new

    df.to_parquet(EMBEDDING_PARQUET, index=False)
    print(f"  ✅ Added {len(records)} embeddings")
...
def process_one_audio(audio_path: str, output_folder: str):
    ...
    try:
        ...
        embedding_records = generate_rag_embeddings(final_output, os.path.basename(audio_path))

        append_embeddings_to_parquet(embedding_records)
